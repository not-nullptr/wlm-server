...
import { MSNStatus } from "../types/User";
import {
	MSNUtils,
...
import { Status } from "../util/Discord";
...
BetaInvites: 1\r\n\r\nUBX 1:${jwt.passport} 0`);
	...
	const guilds = MSNUtils.readyData.guilds;

	const presences = generatePresences([
		...guilds.map((g) => ({
			displayName: g.properties.name,
			id: "0000000000",
			passport: `${g.id}@discord.com`,
			// status: discordStatusToMSNStatus(f.status),
			status: MSNStatus.Offline,
		})),
		...friends.map((f) => {
			const user = MSNUtils.readyData.users.find(
				(u) => u.id === f.user_id!,
			)!;
			const presence = MSNUtils.readyData.merged_presences.friends.find(
				(p) => user.id === p.user_id,
			);
			return {
				...
				// status: discordStatusToMSNStatus(f.status),
				status: discordStatusToMSNStatus(
	...
	const chunk = 12;
	const groups = presences
		.map((_, i, arr) => (i % chunk === 0 ? arr.slice(i, i + chunk) : null))
